merge_dim_tables <- function(dim_data_simp){
  # I suspect I will have to tweak this function to fix the plot
...
#' valid answers than this % will be discarded.
#' @param variables_threshold A numeric between 0 and 100. Variables that have
#' a valid value for fewer than this % of samples will be discarded.
...
filter_for_mfa <- function(pat_data, patients_threshold = 0, variables_threshold = 0) {
  ...
  pat_heatmap[pat_heatmap != 1] <- 0
  answer_pct <- colSums(pat_heatmap) / nrow(pat_heatmap) * 100 # Then we remove variables that fewer than 50 % of patients have answered
  answer_pass <- names(answer_pct[answer_pct >= variables_threshold])
  pat_heatmap <- pat_heatmap[, answer_pass, drop = FALSE]
  patient_pct <- rowSums(pat_heatmap) / ncol(pat_heatmap) * 100 # Lastly we remove patients who have answered fewer than 25 % of remaining questions
  patient_pass <- names(patient_pct[patient_pct >= patients_threshold]) 
  pat_heatmap <- pat_heatmap[patient_pass, , drop = FALSE]
  removed_vars <- initial_vars - ncol(pat_heatmap)
  removed_patients <- initial_patients - nrow(pat_heatmap)
  filtered_data <- pat_data[patient_pass, answer_pass, drop = FALSE]
  if(removed_vars > 0) message(red$bold("WARNING: REMOVED", removed_vars, "VARIABLE(S) THAT DID NOT PASS FILTERING"))
  if(removed_patients > 0) message(red$bold("WARNING: REMOVED", removed_patients, "PATIENT(S) THAT DID NOT PASS FILTERING"))
...
filter_for_factors <- function(pat_data) {
  ...
  factor_data <- pat_data[, valid_variables, drop = FALSE]
  return(factor_data)
}
