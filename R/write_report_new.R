...
write_functional_report_new <- function(hunter_results, 
    ...
    if(is.null(func_results$WGCNA_ORA) && grepl("c|i", report)) {
        stop("cluster reports chosen but no cluster results available")
    }
    ...
    if(grepl("c", report)){
        ...
        message("\tRendering cluster reports")
    ...
    check_and_create_dir(results_temp)
        ...
        if (any(grepl("WGCNA",names(func_results)))) { # Clustered
            ...
            for(cl in cls) {
                print(c("for cluster", cl))
                # cl_enrichments_ORA <- enrichments_ORA_expanded[[cl]]
                cl_flags_ora <- sapply(enrichments_ORA_expanded, function(x) {
            ...
            print("out of Rendering Clusters")
