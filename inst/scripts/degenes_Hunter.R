...
if( Sys.getenv('DEGHUNTER_MODE') == 'DEVELOPMENT' ){
  ...
  custom_libraries <- c('main_degenes_Hunter.R', 'io_handling.R', 'general_functions.R', 'dif_expression_packages.R', 'qc_and_benchmarking_functions.R', 'correlation_packages.R', 'plotting_functions.R', 'write_report.R')
...
option_list <- list(
  ...
  optparse::make_option(c("-b", "--WGCNA_memory"), type="integer", default=5000,
    help="Maximum block size value, to be passed to the blockwiseModules function of WGCNA as the maxBlockSize argument. Default=%default"),
  ...
  optparse::make_option("--WGCNA_deepsplit", type="integer", default=2,
    help="This option control the module building process and is defined as 1,2,3 and 4 values. 1 for rough clustering and 4 for accurate clustering. Default=%default"),
  ...
  optparse::make_option("--WGCNA_mergecutHeight", type="double", default=0.25,
    help="Value to merge two similar modules: Maximum dissimilarity (i.e., 1-correlation). Default=%default"),
