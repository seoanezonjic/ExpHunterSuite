...
option_list <- list(
  ...
  optparse::make_option(c("-f", "--funsys"), type="character", default="BP,MF,CC,KEGG,Reactome", 
                        ...
                        KEGG => KEGG, Reactome => Reactome, DO => Disease Ontology, DGN => DisGeNET. Default=%default"),
  ...
  optparse::make_option(c("-T", "--top_categories"), type="integer", default=NULL,
                        help="For the merged ORA object, only consider the top n results per cluster. Default=%default"),
  ...
  optparse::make_option(c("--cex_label_category"), type="numeric", default=1,
                        help="Values smaller than one will reduce label size, values larger than one will increase it."),
  optparse::make_option(c("--cex_line"), type="numeric", default=1,
                        help="Values smaller than one will reduce line size, values larger than one will increase it."),
  optparse::make_option(c("--cex_pie2axis"), type="numeric", default=1,
                        help="Values smaller than one will reduce pie axis size, values larger than one will increase it."),
...
write_clusters_to_enrichment(output_path = output_path, mode = opt$mode,
  ...
  cex_line = opt$cex_line, cex_pie2axis = opt$cex_pie2axis,
