...
option_list <- list(
  ...
  optparse::make_option(c("-M", "--mode"), type="character", default="R", 
                        help="String indicating report modes to execute. R: Generate report for each cluster and all clusters combined. S: Summary mode (sumamrized categories heatmap)"),
  ...
  optparse::make_option(c("-T", "--top_categories"), type="integer", default=100,
                        help="For the merged ORA object, show maximum this number categories in the plots. Default=%default"),
  ...
  optparse::make_option(c("--size_edge"), type="numeric", default=0.5,
                        help="Size of edges. Smaller values will reduce size, higher will increase it."),
