...
option_list <- list(
	make_option(c("-t", "--target_miRNA_results"), type="character", default=NULL,
		help="Set miRNA-target results folder"),
	make_option(c("-a", "--aproaches"), type="character", default="EE,Eh,Ed,hd,hE",
		help = "zero = All possible correlations between miRNA counts and RNA counts, EE = Anticorrelation between RNAseq modules Eigengene and miRNAseq modules Eigengene, Eh = Anticorrelation between RNAseq modules Eigengene and miRNAseq hub genes, Ed = Anticorrelation between RNAseq modules Eigengene and miRNAseq DEG expression profiles, hd = Anticorrelation between RNAseq hub genes and miRNAseq DEG expression profiles, hE = Anticorrelation between RNAseq hub genes and miRNAseq modules Eigengene. Default : %default"),
	...
	make_option(c("-f", "--func_annot_db"), type="character", default="g,K,R",
 	...
 	make_option(c("-G", "--GO_subont"), type="character", default=c("BP,MF,CC"),
    	help="GO sub-ontologies to use for functional analysis (MF = Molecular Function, BP = Biological Process, CC = Celular Component). Default=%default"), # Not Checked
...
current_organism_info <- subset(all_organisms_info, rownames(all_organisms_info) %in% opt$organism)  
...
if (!"target_results_table.txt" %in% list.files(strategy_folder)) {
	warning(paste0("No files have been fond in results folder for ", strategy, " strategy. Aborting..."))
	next
}
raw_data <- read.table(file.path(strategy_folder, "target_results_table.txt"), header=TRUE, row.names=NULL, sep="\t")
# raw_data <- head(raw_data, 100)
# q()

	unique_genes <- unique(raw_data[!raw_data$entrezgene %in% c("", NA), c("entrezgene", "mean_logFCs")])
...
if (launch_expanded) {
	...
	expanded_targets <- lapply(unique_miRNAs, function(miRNA){
			mirna_targets <- raw_data[raw_data$miRNA_names == miRNA, "entrezgene"]
...
if (launch_expanded) {
	for (subont in GO_subontologies) {
		...
		enrichments_ORA_expanded[[subont_name]] <- enrichment_clusters_ORA(genes = expanded_targets,
						        organism = current_organism_info$Bioconductor_DB[1],
						        keyType = "ENTREZID",
						        pvalueCutoff = opt$pthreshold,
						        pAdjustMethod = "BH",
...
if ("K" %in% nomenclatures) {
	...
	if (launch_default) {
		enrich_KEGG <- enrichment_ORA(genes = enrich_genes, 
								...
								useInternal = TRUE, 
								qvalueCutoff = opt$qthreshold)
		# write.table(enrich_KEGG, file=file.path(paths$root, "KEGG_results"), quote=FALSE, col.names=TRUE, row.names = FALSE, sep="\t")
			
	}
	if (launch_expanded) {
		enrichments_ORA_expanded[["KEGG"]] <- enrichment_clusters_ORA(genes = expanded_targets,
                                 organism = current_organism_info$KeggCode[1],
                                 keyType = "kegg",
                                 pvalueCutoff = opt$pthreshold,
                                 pAdjustMethod = "BH",
...
if ("R" %in% nomenclatures) {
	...
	if (launch_expanded) {
		enrichments_ORA_expanded[["REACT"]] <- enrichment_clusters_ORA(genes = expanded_targets,
                                 organism = current_organism_info$Reactome_ID[1],
                                 keyType = "ENTREZID",
                                 pvalueCutoff = opt$pthreshold,
                                 pAdjustMethod = "BH",
...
if (launch_default) {
	...
	outf <- file.path(output_files, paste0(strategy, "_targets_functional.html"))

	rmarkdown::render(file.path(main_path_script, 'templates', 'targets_functional.Rmd'), output_file = outf, intermediates_dir = output_files)
}
...
if (launch_expanded) {
	...
	if (!is.null(opt$RNAseq_folder) && !is.null(opt$RNAseq_folder)) {
		...
		RNAseq[['normalized_counts']] <- as.data.frame(as.table(scale_data_matrix(data_matrix = RNAseq[['normalized_counts']], transpose = FALSE)))
		colnames(RNAseq[['normalized_counts']]) <- c("Sample","Gene","Count")
		
		# str(miRNAseq[['normalized_counts']])

		miRNAseq[['normalized_counts']] <- scale_data_matrix(data_matrix = miRNAseq[['normalized_counts']], transpose = FALSE)
	...
	for(miRNA in unique_miRNAs) {
		...
		target_outf <- file.path(output_files, paste0("targets_", miRNA,".html"))
		# Generate report
		rmarkdown::render(file.path(main_path_script, 'templates', 'miRNA_target_func.Rmd'), output_file = target_outf, intermediates_dir = output_files)
	}

	message("\tRendering merged miRNA report")
	outf_cls <- file.path(output_files, "expanded_targets_func.html")
	rmarkdown::render(file.path(main_path_script, 'templates', 'targets_global_report.Rmd'),output_file = outf_cls, intermediates_dir = output_files)
}
# 
# }
