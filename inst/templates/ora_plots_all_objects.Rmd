...
if(length(get_genes(enrich_obj)) <= max_genes){
	go_pp <- enrichplot::cnetplot(enrich_obj, showCategory=n_category, foldChange=fc_vector) +
	 		 { if(!is.null(fc_vector)) ggplot2::scale_color_gradient2(name = gene_attribute_name, low = "#0000BF", high = "#bf0000", na.value = "#50EA55") }
	...
	go_pp <- enrichplot::cnetplot(enrich_obj_maxed, showCategory=n_category, foldChange=fc_vector) +
	 		 { if(!is.null(fc_vector)) ggplot2::scale_color_gradient2(name = gene_attribute_name, low = "#0000BF", high = "#bf0000", na.value = "#50EA55") }
	invisible(plot_in_div(go_pp, fig_width = default_width))
}
