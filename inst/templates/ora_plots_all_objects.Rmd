...
```{r "{{funsys}}_plots", echo=FALSE, results='asis', warning=FALSE, message=FALSE}
message("starting")
n_category <- 30
...
if(length(get_genes(enrich_go_ora_symb)) <= 200){
	go_pp <- enrichplot::cnetplot(enrich_go_ora_symb, showCategory=n_category, foldChange=fc_vector) +
...
if (check_categories(enrich_obj, min_categories = 1)) {
...
go_pp <- enrichplot::heatplot(enrich_go_ora_symb, showCategory=n_category, foldChange=fc_vector)
...
if(length(get_categories(enrich_obj))>=2){ # Needs, at least, two categories
