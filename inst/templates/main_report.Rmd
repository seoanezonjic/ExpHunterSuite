...
```{r logFCplots, echo = FALSE, eval=(!grepl("F", opt$modules) && nchar(stringr::str_remove(opt$modules, "W")) > 1), results = "asis"}
pp <- ht2logFCPlot(DE_all_genes, var_filter = var_thr, title = paste0("(variance threshold >= ",var_thr,")"))
	cat(paste0("\nThis graph shows logFC calculated (y-axis) for each package (points) and gene (x-axis). Only genes with variability over ", var_thr, " will be plotted. This representation allows to user to observe the behaviour of each DE package and detect when one of them has atypical results.\n"))
	plot(pp)
```
