...
Sample rank is the position a sample holds after sorting by total gene counts (or total reads, if provided).
...
```{r coverage_plot, echo = FALSE, warning = FALSE}
    ...
    ggplot2::ggplot(coverage_df, ggplot2::aes(x = count_rank, y = total_counts)) +
        ggplot2::geom_point(size = 3) +
...
```{r coverage_plot_frac, eval = !is.null(coverage_df$counted_frac),  echo = FALSE, warning = FALSE}
    ggplot2::ggplot(coverage_df, ggplot2::aes(x = count_rank, y = counted_frac)) +
        ggplot2::geom_point(size = 3) +
        cowplot::theme_cowplot() +
        cowplot::background_grid() +
        ...
        ggplot2::ylim(c(0,NA)) +
        ggplot2::scale_color_brewer(palette="Dark2") +
        ggplot2::ggtitle("Fraction counts read sorted by rank")
```
...
```{r expGenes_plot, echo = FALSE, warning = FALSE}
		colors <- c("Unique" = "blue", "Union" = "red", "Intersection" = "green")
		ggplot2::ggplot(data = expGenesDf, ggplot2::aes(x = count_rank)) +
		    ggplot2::geom_line(ggplot2::aes(y = expressedGenes, color = "Unique")) +
		    ggplot2::geom_line(ggplot2::aes(y = unionExpressedGenes, color = "Union")) +
		    ggplot2::geom_line(ggplot2::aes(y = intersExpressedGenes, color = "Intersection")) +
...
This plot correlates sample rank with sample effective library size (DESeq2 normalization factor)")
```
...
```{r DESeq2_conditional_block_sample_rank_plot, eval="DESeq2" %in% names(all_counts_for_plotting), echo=FALSE, warning = FALSE, message = FALSE}
	...
	effective_lib_size <- effective_lib_size[match(coverage_df$sampleID, names(effective_lib_size))] 
	count_rank <- coverage_df$count_rank
	size_vs_rank <- data.frame(count_rank = count_rank,
    ...
    ggplot2::ggplot(size_vs_rank, ggplot2::aes(x = count_rank, y = eff_lib_size)) +
        ggplot2::geom_point(size = 3) +
        cowplot::theme_cowplot() +
        cowplot::background_grid() +
...
cat("The effective library size is the factor used by DESeq2 normalizatioin algorithm for each sample. The effective library size must be dependent of raw library size.")
```
