...
```{r pca_plot, eval="DESeq2" %in% names(all_counts_for_plotting), echo = FALSE}
...
ggplot2::ggplot(pcaData, ggplot2::aes(x = PC1, y = PC2, color = name, shape = cond)) +
  ...
  ggplot2::geom_text(ggplot2::aes(label=name),hjust=0, vjust=0) +
...
```{r logFCplots, echo = FALSE}
pp <- ht2logFCPlot(DE_all_genes, var_filter = var_thr, title = paste0("(variance threshold >= ",var_thr,")"))
if(!is.null(pp)){
	cat(paste0("This graph shows logFC calculated (y-axis) for each package (points) and gene (x-axis). Only genes with variability over ", var_thr, " will be plotted. This representation allows to user to observe the behaviour of each DE package and detect when one of them has atypical results."))
	plot(pp)
}
```
...
mod_traits_net <- corM2igraph(corME2, cor_abs_thr = 0.85)
...
if (nrow(mod_traits_net$nodes < 2)){
	...
	cat("This plot shows modules (black) and factors (yellow) as nodes. Correlations coefficients over 0.85 (red) and under -0.85 (blue) are represented as edges\n")
	"%>%" <- magrittr::"%>%"
	...
	# ggraph::geom_edge_diagonal(alpha = .2, color='white') + 
	...
	# igraph::plot.igraph(mod_net, 
	# 	                edge.color = ifelse(mod_traits_net$edges$corr_type == "corr", "#FF0000","#0000FF"), 
	# 	                vertex.size=10 ,
	# 	                # layout = layout, 
	# 	                vertex.color = ifelse(grepl("_",mod_traits_net$nodes$nodes), "#43CF4C", "white"), 
...
```{r PCIT_conditional_block_plot, eval=grepl("P", opt$modules), echo=FALSE, warning=FALSE, message=FALSE}
    ggplot2::ggplot(metrics_pcit, ggplot2::aes(x=ctrl_cn)) +
              ggplot2::geom_density()
    ggplot2::ggplot(metrics_pcit, ggplot2::aes(x=treat_cn)) +
              ggplot2::geom_density()
    ggplot2::ggplot(metrics_pcit, ggplot2::aes(x=diff_cn)) +
            ggplot2::geom_density()
    ggplot2::ggplot(metrics_pcit, ggplot2::aes(x=diffK)) +
            ggplot2::geom_density()

    av_de <- ggplot2::ggplot(metrics_pcit, ggplot2::aes(x=average, y=de)) +
            ggplot2::geom_point()
    ...
    de_diffK <- ggplot2::ggplot(metrics_pcit, ggplot2::aes(x=de, y=diffK)) +
            ggplot2::geom_point()
    ...
    de_pif <- ggplot2::ggplot(metrics_pcit, ggplot2::aes(x=de, y=pif)) +
            ggplot2::geom_point()
    ...
    de_rif1 <- ggplot2::ggplot(metrics_pcit, ggplot2::aes(x=de, y=rif1)) +
            ggplot2::geom_point()
    ...
    de_rif2 <- ggplot2::ggplot(metrics_pcit, ggplot2::aes(x=de, y=rif2)) +
            ggplot2::geom_point()
    ...
    rif1_rif2 <- ggplot2::ggplot(metrics_pcit, ggplot2::aes(x=rif1, y=rif2)) +
            ggplot2::geom_point()
