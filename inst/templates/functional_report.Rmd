...
invisible(lapply(graph_files,function(file_name){
	if(DEgenesHunter:::exists_graph_file(path = results_path, file_name = file_name)){
...
invisible(lapply(go_enrich_vars,function(var_name){
	go_flags[var_name] <<- DEgenesHunter:::exists_enrich_df(var_name)
}))
# Check GO section
...
if(go_flags$GO_CL_SECTION){
	...
	for (onto in names(go_main_enrich)) {
		...
		if(go_flags$enrich_go){
			go_flags[paste0("perform_ora_go_",onto)] <- DEgenesHunter:::check_results(enrich_go[[onto]])
		}else if(go_flags$enrich_go_gsea){
			go_flags[paste0("perform_gsea_go_",onto)] <- DEgenesHunter:::check_results(enrich_go_gsea[[onto]])
		}
	}
}
...
invisible(lapply(kegg_enrich_vars,function(var_name){
	kegg_flags[var_name] <<- DEgenesHunter:::exists_enrich_df(var_name)
}))
...
invisible(lapply(react_enrich_vars,function(var_name){
	react_flags[var_name] <<- DEgenesHunter:::exists_enrich_df(var_name)
}))
...
if(!is.null(custom_enrichments)){
	...
	invisible(lapply(custom_enrichments,function(var){
		custom_flags[var$File] <<- DEgenesHunter:::check_results(var$Result)
	}))
...
default_width <- DEgenesHunter:::set_default_width(enrich_obj = enrich_ora, showCategory=n_category)
# Main label 
...
DEgenesHunter:::plot_in_div(pp, fig_width = default_width)
...
if(length(DEgenesHunter:::get_genes(enrich_ora_symb)) <= 200){
...
if (DEgenesHunter:::check_categories(enrich_ora, min_categories = 1)) {
...
width_size <- DEgenesHunter:::calc_width(enrich_ora_symb)
height_size <- DEgenesHunter:::calc_height(enrich_ora_symb) 

DEgenesHunter:::plot_in_div(pp, height_size, width_size)
...
if(length(DEgenesHunter:::get_categories(enrich_ora))>=2){ # Needs, at least, two categories
...
default_width <- DEgenesHunter:::set_default_width(enrich_obj = enrich_gsea, showCategory=n_category)
...
if(length(DEgenesHunter:::get_genes(enrich_gsea_symb)) <= 200){
...
if (DEgenesHunter:::check_categories(enrich_gsea, min_categories = 1)) {
...
width_size <- DEgenesHunter:::calc_width(enrich_gsea_symb)
height_size <- DEgenesHunter:::calc_height(enrich_gsea_symb) 
DEgenesHunter:::plot_in_div(pp, height_size, width_size)
...
for (onto in names(go_main_enrich)) {
	...
	if(go_flags[[paste0("perform_ora_go_",onto)]]){
		default_width <- DEgenesHunter:::set_default_width(enrich_obj = enrich_go[[onto]], showCategory=n_category)
		...
		DEgenesHunter:::plot_in_div(go_pp, fig_width = default_width)
		...
		if(length(DEgenesHunter:::get_genes(enrich_go_ora_symb)) <= 200){
		...
		if (DEgenesHunter:::check_categories(enrich_go[[onto]], min_categories = 1)) {
		...
		width_size <- DEgenesHunter:::calc_width(enrich_go_ora_symb)
		height_size <- DEgenesHunter:::calc_height(enrich_go_ora_symb) 
		DEgenesHunter:::plot_in_div(go_pp, height_size, width_size)

		cat("\n\n**Upsetplot**\n\n", 
			"Top functional terms are listed the Y axis with the number of genes associated to them (Set size). Each column of dots marks one cluster of functional terms. The X axis indicates how many genes are shared exclusively for the members of one cluster.\n\n"
		)
		if(length(DEgenesHunter:::get_categories(enrich_go[[onto]]))>=2){ # Needs, at least, two categories
	...
	if(go_flags[[paste0("perform_gsea_go_",onto)]]){
		# Add main label
		default_width <- DEgenesHunter:::set_default_width(enrich_obj = enrich_go_gsea[[onto]], showCategory=n_category)
		...
		if(length(DEgenesHunter:::get_genes(enrich_go_gsea_symb)) <= 200){
		...
		if (DEgenesHunter:::check_categories(enrich_go_gsea[[onto]], min_categories = 1)) {
		...
		width_size <- DEgenesHunter:::calc_width(enrich_go_gsea_symb)
		height_size <- DEgenesHunter:::calc_height(enrich_go_gsea_symb) 
...
if(go_flags$enrich_go){
	...
	# Concat results
	enr_go <- enrich_go[[1]]
	default_width <- DEgenesHunter:::set_default_width(enrich_obj = enr_go, showCategory=n_category)
	...
	if(length(DEgenesHunter:::get_genes(enr_go_symb)) <= 200){
	...
	if (DEgenesHunter:::check_categories(enr_go, min_categories = 1)) {
	...
	width_size <- DEgenesHunter:::calc_width(enr_go_symb)
	height_size <- DEgenesHunter:::calc_height(enr_go_symb) 
	DEgenesHunter:::plot_in_div(go_pp, height_size, width_size)

	cat("\n\n**Upsetplot**\n\n", 
		"Top functional terms are listed the Y axis with the number of genes associated to them (Set size). Each column of dots marks one cluster of functional terms. The X axis indicates how many genes are shared exclusively for the members of one cluster.\n\n"
	)
	if(length(DEgenesHunter:::get_categories(enr_go))>=2){ # Needs, at least, two categories
...
if(go_flags$enrich_go_gsea){
	...
	default_width <- DEgenesHunter:::set_default_width(enrich_obj = enr_go_gsea, showCategory=n_category)
	...
	if(length(DEgenesHunter:::get_genes(enr_go_gsea_symb)) <= 200){
	...
	if (DEgenesHunter:::check_categories(enr_go_gsea, min_categories = 1)) {
	...
	width_size <- DEgenesHunter:::calc_width(enr_go_gsea_symb)
	height_size <- DEgenesHunter:::calc_height(enr_go_gsea_symb)
...
default_width <- DEgenesHunter:::set_default_width(enrich_obj = enrich_react, showCategory=n_category)
...
if(length(DEgenesHunter:::get_genes(enrich_react_symb)) <= 200){
...
if (DEgenesHunter:::check_categories(enrich_react, min_categories = 1)) {
...
width_size <- DEgenesHunter:::calc_width(enrich_react_symb)
height_size <- DEgenesHunter:::calc_height(enrich_react_symb)
DEgenesHunter:::plot_in_div(pp, height_size, width_size)
...
if(length(DEgenesHunter:::get_categories(enrich_react))>=2){ # Needs, at least, two categories
...
default_width <- DEgenesHunter:::set_default_width(enrich_obj = enrich_react_gsea, showCategory=n_category)
# Main label
cat("\n\n### **Gene Set Enrichment Analysis**\n",
	"In this method, all the genes are sorted by their fold-change and the algorithm scan which genes with similar fold-change shares a term of the selected functional category.\n\n"
)
...
if(length(DEgenesHunter:::get_genes(enrich_react_gsea_symb)) <= 200){
...
if (DEgenesHunter:::check_categories(enrich_react_gsea, min_categories = 1)) {
...
width_size <- DEgenesHunter:::calc_width(enrich_react_gsea_symb)
height_size <- DEgenesHunter:::calc_height(enrich_react_gsea_symb)
DEgenesHunter:::plot_in_div(pp, height_size, width_size)
...
invisible(lapply(seq_along(custom_enrichments),function(i){
	...
	if(!custom_flags[[res$File]]){
		...
		body <- "enr_custom <- custom_enrichments[[{{i}}]]$Result
				default_width <- DEgenesHunter:::set_default_width(enrich_obj = enr_custom, showCategory=n_category)
				...
				if(length(DEgenesHunter:::get_genes(enr_custom)) <= 200){
				...
				if (DEgenesHunter:::check_categories(enr_custom, min_categories = 1)) {
				...
				width_size <- DEgenesHunter:::calc_width(enr_custom)
				height_size <- DEgenesHunter:::calc_height(enr_custom)
				DEgenesHunter:::plot_in_div(pp, height_size, width_size)
					
				cat('\n\n**Upsetplot**\n\n', 
					'Top functional terms are listed the Y axis with the number of genes associated to them (Set size). Each column of dots marks one cluster of functional terms. The X axis indicates how many genes are shared exclusively for the members of one cluster.\n\n'
				)
				if(length(DEgenesHunter:::get_categories(enr_custom))>=2){ # Needs, at least, two categories
