...
# Check the are ACTUALLY POSITIVE
topgenes <- topgenes[topgenes$mean_logFCs > 0,]
botgenes <- botgenes[botgenes$mean_logFCs < 0,]
...
if (check_categories(enrich_ora, min_categories = 1) && nrow(as.data.frame(enrich_ora)) > 1 ) {
...
pp <- enrichplot::gseaplot2(enrich_gsea, geneSetID=ifelse(nrow(enrich_gsea@result) < 4, seq(nrow(enrich_gsea@result)), seq(4)), title = enrich_gsea$Description[1], pvalue_table = TRUE)
...
for (onto in names(go_main_enrich)) {
	...
	if(go_flags[[paste0("perform_gsea_go_",onto)]]){
		...
		go_pp <- enrichplot::gseaplot2(enrich_go_gsea[[onto]], geneSetID=ifelse(nrow(enrich_go_gsea[[onto]]@result) < 4, seq(nrow(enrich_go_gsea[[onto]]@result)), seq(4)), pvalue_table = TRUE) 
		plot_in_div(go_pp, fig_width = default_width)
		cat("\n \n")
	}
}
...
pp <- enrichplot::gseaplot2(enrich_react_gsea, geneSetID=ifelse(nrow(enrich_react_gsea@result) < 4, seq(nrow(enrich_react_gsea@result)), seq(4)), title = enrich_react_gsea$Description[1], pvalue_table = TRUE)
plot_in_div(pp, fig_width = default_width)
