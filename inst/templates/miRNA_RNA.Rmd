---
author: "SysBioLab"
output:
  html_document:
    toc: true
    toc_float: true
    df_print: paged
    fig_width: 12
---

<style type='text/css'>
    body .main-container { 
        max-width: 90%;
    
    }

    .plot_real_size {
      overflow: scroll;
      max-height: 600px;
    }

    .plot_real_size img{
      max-width: none;
      max-height: none;
    }

    embed {
   
  }   
</style>


# **RNAseq - miRNAseq correlation report**

```{r introduction, echo=FALSE, results = 'asis', warning = FALSE, message = FALSE}
cat(paste0("This report summarizes results from [script_name] execution comparing **", RNAseq_folder, "** RNAseq data and **", miRNAseq_folder, "** miRNAseq data.
 miRNAseq and RNAseq data has been correlated using different strategies and then compared with databases included in multiMiR.\n\n" ))
```


* Strategies description: 
    + counts_RNA_vs_counts_miRNA correlates normalized RNAseq counts with normalized miRNAseq counts. 
    + Eigen_RNA_v_Eigen_miRNA correlates Eigengene profile of RNAseq coexpression modules with Eigengene profile of miRNAseq coexpression modules. 
    + Eigen_RNA_v_Hub1_miRNA correlates Eigengene profile of RNAseq coexpression modules with hub gene profile of miRNAseq coexpression modules. 
    + Eigen_RNA_v_counts_miRNA correlates Eigengene profile of RNAseq coexpression modules with with normalized miRNAseq counts.
    + Hub1_RNA_v_counts_miRNA correlates hub gene profile of RNAseq coexpression modules with with normalized miRNAseq counts.
    + Hub1_RNA_v_Eigen_miRNA correlates hub gene profile of RNAseq coexpression modules with Eigengene profile of miRNAseq coexpression modules.

## **RNAseq and miRNAseq correlation methods comparison**

This plot shows the distributions of correlations between RNAseq and miRNAseq using diferent strategies.
```{r methods_comparison_violin_cor, echo=FALSE, results = 'asis', warning = FALSE, message = FALSE}

all_cor_dist <- strategies
all_cor_dist[["all_possible_pairs"]] <- NULL
strat_names <- names(all_cor_dist)
all_cor_dist <- lapply(all_cor_dist, function(strategy_data){
   dist <- strategy_data[,c("correlation", "pval")]
 
   return(dist)
  })
names(all_cor_dist) <- strat_names


all_cor_dist <- as.data.frame(data.table::rbindlist(all_cor_dist, use.names=TRUE, idcol= "strategy"))

pp <- ggplot2::ggplot(all_cor_dist, ggplot2::aes(x = strategy, y = correlation, fill = strategy)) +
  ggplot2::geom_violin(trim = FALSE, scale = "width",draw_quantiles = c( 0.1, 0.05)) + #ggplot2::ylim(-1.000001, min + 0.00001) +
  ggplot2::geom_hline(yintercept= corr_cutoff) +
  ggplot2::geom_boxplot(width=0.05)+
  ggplot2::theme(axis.text.x = ggplot2::element_blank(), axis.title.x = ggplot2::element_blank()) 
pp

```

```{r methods_comparison_violin_p, echo=FALSE, results = 'asis', warning = FALSE, message = FALSE, eval = FALSE}

cat("\n\nThis plot shows the distributions of correlation P values between RNAseq and miRNAseq using diferent strategies.
  \n")
pp <- ggplot2::ggplot(all_cor_dist, ggplot2::aes(x = strategy, y = pval, fill = strategy)) +
  ggplot2::geom_violin(trim = FALSE, scale = "width") + ggplot2::ylim(-0.0001, 0.1) +
  ggplot2::geom_boxplot(width=0.05)+
  ggplot2::theme(axis.text.x = ggplot2::element_blank(), axis.title.x = ggplot2::element_blank()) 
pp

```


## **Overlapping between pairs and databases**
This plot sumarizes the pairs of a putative nobel miRNA and the pairs of a miRNA found on miRBASE. 

* The pairs with known miRNAs were searched in: 
  + All multiMiR databases (multiMiR)
  + Only prediction databases (predicted)
  + Only validated databases (validated)
  + Intersection of prediction and validated databases (both)

Overlapping of significant known pairs along the databases were compared with a random subset of significant pairs count from background pairs (all possible relations between RNAseq and miRNA seq input data). 
The random subsets are tagged as "_random".


```{r methods_comparison_bar, echo=FALSE, results = 'asis', warning = FALSE, message = FALSE}


pp <- ggplot2::ggplot(filters_summary, ggplot2::aes(y = pairs, x = strategy, fill = type)) + 
      ggplot2::geom_bar(stat= "identity",position=ggplot2::position_dodge()) +
      ggplot2::geom_errorbar(ggplot2::aes(ymin=pairs-sdev, ymax=pairs+sdev), width=.2,
                 position=ggplot2::position_dodge(.9)) + 
      ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 20, hjust = 1)) + 
      ggplot2::geom_text(ggplot2::aes(label = pairs), angle = 45, position = ggplot2::position_dodge(width=0.95), size=2, vjust = -2)
pp
```

## **Overlapping significance**
This table shows P values of comparing significant pairs in databases with random distribution (as normal distributin). 

```{r methods_comparison_pval, echo=FALSE, results = 'asis', warning = FALSE, message = FALSE}
filter_stats <- filters_summary[!is.na(filters_summary$p_val), c("strategy", "type", "p_val", "quantile")]
filter_stats <- stats::reshape(filter_stats, timevar="type", idvar="strategy", direction ="wide")
filter_stats$targets <- rep(NA, nrow(filter_stats))
prevalent_miRNAs <- miRNAseq[["DH_results"]]$gene_name[miRNAseq[["DH_results"]]$genes_tag == "PREVALENT_DEG"]
for (strategy in unique(filter_stats$strategy)) {
  correlated_pairs <- strategies$all_possible_pairs[strategies[[strategy]]$index, c("miRNAseq", "RNAseq")]
  filter_stats[filter_stats$strategy == strategy, "miRNAs"] <- length(unique(correlated_pairs$miRNAseq))
  filter_stats[filter_stats$strategy == strategy, "targets"] <- length(unique(correlated_pairs$RNAseq))
  filter_stats[filter_stats$strategy == strategy, "DEMs"] <- length(unique(correlated_pairs[correlated_pairs$miRNAseq %in% prevalent_miRNAs, "miRNAseq"]))
  filter_stats[filter_stats$strategy == strategy, "DEMs_targets"] <- length(unique(correlated_pairs[correlated_pairs$miRNAseq %in% prevalent_miRNAs, "RNAseq"]))
}
 
  DT::datatable(filter_stats, filter = 'top', rownames = TRUE, extensions = c('Buttons','ColReorder'),
            options = list(
              colReorder = TRUE,
              dom = 'lftBip',
              buttons = c('copy', 'csv', 'excel')
        ))

valid_strats <- filter_stats[filter_stats$p_val.predicted < 0.01 & filter_stats$p_val.validated < 0.01, "strategy"]

```

## **Association with databases**
This table shows the association of significant pairs and pairs from multiMiR databases. The association is measured by one tailed Fisher s exast test.
Lower P value shows that significant correlated pairs are enriched on pairs in databases.

```{r fet_table,  echo=FALSE, warning = FALSE, message = FALSE}
    DT::datatable(as.data.frame(fet_pvals), filter = 'top', rownames = TRUE, extensions = c('Buttons','ColReorder'),
            options = list(
              colReorder = TRUE,
              dom = 'lftBip',
              buttons = c('copy', 'csv', 'excel')
        ))
```

## **Likelihood ratio for possitive test**  

```{r LR_test,  echo=FALSE, results = 'asis', warning = FALSE, message = FALSE}
    DT::datatable(as.data.frame(LR_test), filter = 'top', rownames = TRUE, extensions = c('Buttons','ColReorder'),
            options = list(
              colReorder = TRUE,
              dom = 'lftBip',
              buttons = c('copy', 'csv', 'excel')
        ))
```

## **Likelihood ratio for possitive subjects**

```{r LR_sub,  echo=FALSE, results = 'asis', warning = FALSE, message = FALSE}
    DT::datatable(as.data.frame(LR_sub), filter = 'top', rownames = TRUE, extensions = c('Buttons','ColReorder'),
            options = list(
              colReorder = TRUE,
              dom = 'lftBip',
              buttons = c('copy', 'csv', 'excel')
        ))
```


## **Strategies overlapping**
This plot shows shared significant pairs between strategies

```{r methods_comparison_intersection, echo=FALSE, results = 'asis', warning = FALSE, message = FALSE}


common_pairs <- matrix(,nrow = length(approaches), ncol = length(approaches))

colnames(common_pairs) <- approaches
rownames(common_pairs) <- approaches

for (col_strategy in approaches) {
  strat_pairs <- strategies$all_possible_pairs[all_strategies[all_strategies$strategy == col_strategy, "pair_n"], c("RNAseq", "miRNAseq")]

  col_pairs <- strat_pairs %>% tidyr::unite("pairs", RNAseq:miRNAseq, sep = "", na.rm = FALSE)
  col_pairs <- col_pairs$pairs
  for (row_strategy in approaches) {
    strat_pairs <- strategies$all_possible_pairs[all_strategies[all_strategies$strategy == row_strategy, "pair_n"], c("RNAseq", "miRNAseq")]
    row_pairs <- strat_pairs %>% tidyr::unite("pairs", RNAseq:miRNAseq, sep = "", na.rm = FALSE)
    row_pairs <- row_pairs$pairs
    common_pairs[row_strategy, col_strategy] <- length(intersect(col_pairs, row_pairs))
  }
}

common_pairs <- as.data.frame(as.table(as.matrix(common_pairs)))

names(common_pairs) <- c("x_axis", "y_axis",  "intersection")

common_pairs$text <- as.character(common_pairs$intersection)
common_pairs$x_axis <- as.character(common_pairs$x_axis)
common_pairs$y_axis <- as.character(common_pairs$y_axis)



pp <- gg_heatmap(data =  common_pairs, 
            x_axis = "x_axis",
            y_axis= "y_axis",
            fill = "intersection",
            text_plot= "text", 
            labs = FALSE)
pp

```

```{r roc_curve, echo=FALSE, results = 'asis', warning = FALSE, message = FALSE, eval = FALSE}
  print(valid_strats)
  print(roc_data)
    roc_predicted <- roc_data$predicted
    roc_data$predicted <- NULL
    roc_validated <- roc_data$validated
    roc_data$validated <- NULL
    roc_data <- roc_data[, valid_strats]


    # roc_data <- as.matrix(roc_data)
    cat("\n\nROC curve using multiMiR predicted relations as gold standard\n\n")
    pred_pred <- ROCR::prediction(roc_data, labels = matrix(roc_predicted, nrow = length(roc_predicted), ncol = ncol(roc_data)))
    perf_roc_pred <- ROCR::performance(pred_pred, "tpr", "fpr")

    plot(perf_roc_pred, main = "ROC")
    cat("\n\nPrecision-Recall curve using multiMiR predicted relations as gold standard\n\n")

    prec_rec_pred <- ROCR::performance(pred_pred, "prec", "rec")
    plot(prec_rec_pred, main = "PREC-REC")

    cat("\n\nROC curve using multiMiR validated relations as gold standard\n\n")

    pred_val <- ROCR::prediction(roc_data, labels = matrix(roc_validated, nrow = length(roc_validated), ncol = ncol(roc_data)))
    perf_roc_val <- ROCR::performance(pred_val, "tpr", "fpr")
    plot(perf_roc_val, main = "ROC")  

    prec_rec_val <- ROCR::performance(pred_val, "prec", "rec")
    plot(prec_rec_val, main = "PREC-REC")

``` 

```{r all_db_dist, echo=FALSE, results = 'asis', warning = FALSE, message = FALSE, eval = FALSE}
    cat("\n\nArea under the curve for different strategies using multiMiR predicted and validated pairs as gold standard\n\n")
    predicted_auc <- ROCR::performance(pred_pred, "auc")
    predicted_val <- ROCR::performance(pred_val, "auc")
    all_auc <- data.frame(strategy = colnames(roc_data),
                          predicted_auc = unlist(predicted_auc@y.values),
                          validated_auc = unlist(predicted_val@y.values)
                          )

    DT::datatable(all_auc, filter = 'top', rownames = FALSE, extensions = c('Buttons','ColReorder'),
            options = list(
              paging = FALSE,
              colReorder = TRUE,
              dom = 'lftBip',
              buttons = c('copy', 'csv', 'excel')
          ))

```

```{r score_comp_pval, results = 'asis', echo = FALSE, warning = FALSE, message = FALSE}
   DT::datatable(score_comp_pval, filter = 'top', rownames = FALSE, extensions = c('Buttons','ColReorder'),
            options = list(
              paging = FALSE,
              colReorder = TRUE,
              dom = 'lftBip',
              buttons = c('copy', 'csv', 'excel')
          ))

```
## **Significant pairs score**
Each prediction from multiMiR databases has a prediction score that define the significance of miRNA-target prediction.
Score distribution of significant correlated pairs were compared with a random score distribution of each database. 
Significant and random distributions were compared with a one-tiled T test and one-tailed bootstrap T test. 
P values were integrated with Fisher method.   
This plot shows -log(P) and red line indicate -log(0.05).

```{r score_comp_pval_log, results = 'asis', echo = FALSE, warning = FALSE, message = FALSE}
  ggplot2::ggplot(log_score_comp, ggplot2::aes(x = database, y = p.value, fill = strategy, shape = test_type)) +
  ggplot2::geom_line() + 
  ggplot2::geom_point() +
  ggplot2::geom_line(ggplot2::aes(y = -log(0.05), color = "red")) +
  ggplot2::geom_text(ggplot2::aes(y= -log(0.05) , label = "-log(0.05)"), vjust= -0.3)
```




```{r score_comp_bootpval, results = 'asis', echo = FALSE, warning = FALSE, message = FALSE}
# save(score_comp_pval, score_comp_boot_pval, file = "/mnt/scratch/users/bio_267_uma/josecordoba/NGS_projects/pmm2_belen/target_miRNA_2020/test.RData")
   DT::datatable(score_comp_boot_pval, filter = 'top', rownames = FALSE, extensions = c('Buttons','ColReorder'),
            options = list(
              paging = FALSE,
              colReorder = TRUE,
              dom = 'lftBip',
              buttons = c('copy', 'csv', 'excel')
          ))

```
```{r db_distr, results = 'asis', echo = FALSE, warning = FALSE, message = FALSE, eval = FALSE}

    gg <- ggplot2::ggplot(db_distribution, ggplot2::aes(y = score, x = strategy, fill = step)) +
    ggplot2::geom_boxplot() + ggplot2::ylim(0, NA) + ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 20, hjust = 1))

    plot(gg)


```
```{r db_distr_table, results = 'asis', echo = FALSE, warning = FALSE, message = FALSE, eval = FALSE}
   dist_pval <- prediction_dist_pval(db_distribution)
    DT::datatable(dist_pval, filter = 'top', rownames = TRUE, extensions = c('Buttons','ColReorder'),
            options = list(
              colReorder = TRUE,
              dom = 'lftBip',
              buttons = c('copy', 'csv', 'excel')
        ))

```
```{r str_diff_predicted, results = 'asis', echo = FALSE, warning = FALSE, eval = FALSE}
  cat("\n\nmiRNA-targets pairs \n\n")
  venn_data <- lapply(valid_strats, function(strategy){
    strat_significant_pairs <- all_strategies$strategy == strategy & all_strategies$predicted_c > 0

    pairs <- strategies$all_possible_pairs[all_strategies[strat_significant_pairs, "pair_n"], c("RNAseq", "miRNAseq")] #<- paste0(all_strategies[strat_significant_pairs, "miRNAseq"], all_strategies[strat_significant_pairs, "RNAseq"])
    pairs <- pairs %>% tidyr::unite("pair", RNAseq:miRNAseq, sep = "", na.rm = FALSE) 
    pairs <- unique(pairs$pairs)
    return(pairs)
    })
  names(venn_data) <- valid_strats
  grid::grid.draw(VennDiagram::venn.diagram(x = venn_data,
  filename = NULL))
```

```{r str_diff_mirna, results = 'asis', echo = FALSE, warning = FALSE,  eval = FALSE}
  cat("\n\nmiRNA shared\n\n")

  venn_data <- lapply(valid_strats, function(strategy){
    strat_significant_pairs <- all_strategies$strategy == strategy & all_strategies$predicted_c > 0

    miRNA_ID <- unique(strategies$all_possible_pairs[all_strategies[strat_significant_pairs, "pair_n"], "miRNAseq"])
    return(miRNA_ID)
    })
  names(venn_data) <- valid_strats
  grid::grid.draw(VennDiagram::venn.diagram(x = venn_data,
  filename = NULL))
```
```{r str_diff_target, results = 'asis', echo = FALSE, warning = FALSE,  eval = FALSE}
  cat("\n\ntargets shared\n\n")

  venn_data <- lapply(valid_strats, function(strategy){
    strat_significant_pairs <- all_strategies$strategy == strategy & all_strategies$predicted_c > 0

    RNA_ID <- unique(strategies$all_possible_pairs[all_strategies[strat_significant_pairs, "pair_n"], "RNAseq"])
    return(RNA_ID)
    })
  names(venn_data) <- valid_strats
  grid::grid.draw(VennDiagram::venn.diagram(x = venn_data,
  filename = NULL))
```

```{r prec_rec, echo=FALSE, results = 'asis', warning = FALSE, message = FALSE}
    
    prec_recall$gold_standard <- factor(prec_recall$gold_standard, levels=c("predicted","validated","both"))

    for (metric in c("precision", "recall", "F1")) {
      gg <- ggplot2::ggplot(prec_recall, ggplot2::aes(x = strategy, y = prec_recall[,metric], fill = gold_standard))+
      ggplot2::geom_bar(stat= "identity",position=ggplot2::position_dodge()) + 
      ggplot2::ggtitle(metric) + ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 20, hjust = 1))
      cat(paste0("\n\n",metric," comparison for strategies\n\n"))
      plot(gg)
    }


```


```{r render_partial_reports, echo = FALSE, warning = FALSE}
  curr_out <- ""

  curr_out <- unlist(lapply(approaches ,function(method_name){
      sec_text <- paste(c("## **", method_name ," correlation method**\n"),collapse="") # Header
      sec_text <- c(sec_text, knitr::knit_expand("partial/miRNA_cor_methods.Rmd")) # Body
      return(sec_text)
  }))    

``` 

`r paste(knitr::knit(text = curr_out), collapse = '\n')`



