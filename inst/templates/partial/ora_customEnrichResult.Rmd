...
n_category <- 30
default_width <- DEgenesHunter:::set_default_width(enrich_obj = custom_cls_ORA_expanded[["{{cat}}"]][["{{cl}}"]], showCategory=n_category)
if(length(DEgenesHunter:::get_categories(custom_cls_ORA_expanded[["{{cat}}"]][["{{cl}}"]]))>=2){ # Needs, at least, two categories
	# Main label
	fname <- basename("{{cat}}") 
	...
	if(length(DEgenesHunter::get_genes(enrichments_ORA_symb)) <= 150){
	...
	width_size <- DEgenesHunter:::calc_width(enrichments_ORA_symb)
	height_size <- DEgenesHunter:::calc_height(enrichments_ORA_symb) 
