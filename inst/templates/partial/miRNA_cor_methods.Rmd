...
# => method_name :: method name
	...
	method_table <- cbind(method_table,strategies$all_possible_permutations[method_table$pair_n, c("miRNAseq", "RNAseq")])
	...
	if (nrow(method_table) > 0) {
		...
		method_table$raw_cor <- strategies[[ref_strategy]][method_table$pair_n, "correlation"]
		method_table$raw_pval <- strategies[[ref_strategy]][method_table$pair_n, "pval"]
...
```{r "{{method_name}}_score_dist", echo = FALSE, message=FALSE, warning=FALSE, eval = nrow(method_table) > 0}
	...
	# save(list = ls(all.names = TRUE), file = file.path(output_files, "test.RData"), envir = environment())
