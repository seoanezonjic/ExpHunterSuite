...
* **Prevalent DEG:** Genes considered as differentially expressed (DE) by at least `r plotter$hash_vars$opt$minpack_common` packages, as specified by the `minpack_common` argument.
...
This barplot shows the total number of genes passing each stage of analysis - from the total number of genes in the input table of counts, to the genes surviving the expression filter, to the genes detected as DE by one package, to the genes detected by at least `r plotter$hash_vars$opt$minpack_common` packages.
...
```{r logFCplots, echo = FALSE, eval=(!grepl("F", plotter$hash_vars$opt$modules) && sum(grepl("logFC_", colnames(plotter$hash_vars$DE_all_genes))) > 1), results = "asis"}
  ...
  pp <- ht2logFCPlot(plotter$hash_vars$DE_all_genes, var_filter = var_thr, title = paste0("(variance threshold >= ",var_thr,")"))
	print(pp)
```
...
The red horizontal line represents the chosen FDR threshold of `r plotter$hash_vars$opt$p_val_cutoff`. The black lines represent
other values.
...
evp <- EnhancedVolcano::EnhancedVolcano(volcano_DE_all_genes, 
	...
	boxedLabels = TRUE,     
	drawConnectors = TRUE,     
	pCutoff = plotter$hash_vars$opt$p_val_cutoff,     
	FCcutoff = plotter$hash_vars$opt$lfc,  
	cutoffLineType = 3,
...
```{r config_PCA_DEA, echo = FALSE, results = "asis", warning = TRUE, message = TRUE, eval = !is.null(plotter$hash_vars$PCA_res$DEGs)}
...
```{r child=file.path(template_folder,"facto_miner.Rmd"), eval = !is.null(plotter$hash_vars$PCA_res$DEGs)}
...
```{r DESeq2_conditional_block_sample_rank_head, eval="DESeq2" %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}

cat("### **DESeq2 size factor vs. sample rank**")
```

```{r DESeq2_conditional_block_sample_rank_plot, eval="DESeq2" %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, warning = FALSE, message = FALSE}
...
```{r DESeq2_conditional_block_sample_rank_text, eval="DESeq2" %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}
...
```{r DESeq2_conditional_block_ELS_head, eval="DESeq2" %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}
...
```{r DESeq2_conditional_block_ELS_plot, eval="DESeq2" %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, warning = FALSE, message = FALSE}
	effective_lib_size <- DESeq2::sizeFactors(plotter$hash_vars$package_objects[['DESeq2']][['DESeq2_dataset']])
...
```{r DESeq2_conditional_block_ELS_text, eval="DESeq2" %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}

cat("The effective library size is the factor used by DESeq2 normalization algorithm for each sample. The effective library size must be dependent of raw library size.")
```


```{r DESeq2_conditional_block_ma_head, eval='DESeq2' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}
...
```{r DESeq2_conditional_block_ma_plot, eval='DESeq2' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE}
	DESeq2::plotMA(plotter$hash_vars$package_objects[['DESeq2']][['de_deseq2']], cex.lab=1.6, cex.axis=1.5)
```

```{r DESeq2_conditional_block_ma_text, eval='DESeq2' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}
...
```{r DESeq2_only_prev_norm_head, eval="DESeq2" %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}
...
```{r DESeq2_only_prev_norm_plot, eval="DESeq2" %in% names(plotter$hash_vars$all_counts_for_plotting), echo = FALSE, fig.height=12, fig.width=12}
...
```{r edgeR_conditional_block_head, eval='edgeR' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}
...
```{r edgeR_conditional_block_plot, eval='edgeR' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE}

with(plotter$hash_vars$all_counts_for_plotting[['edgeR']], plot(logCPM, logFC, pch=20, main='edgeR: Fold change vs abundance', cex.lab=1.5, cex.axis=1.5))
with(subset(plotter$hash_vars$all_counts_for_plotting[['edgeR']], FDR < plotter$hash_vars$opt$p_val_cutoff), points(logCPM, logFC, pch=20, col='red'))
abline(h=c(-1,1), col='blue')
	
```

```{r edgeR_conditional_block_body, eval='edgeR' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}
...
```{r limma_conditional_block_head, eval='limma' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}
...
```{r limma_conditional_block_plot, eval='limma' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, warning=FALSE}
...
plot(x=plotter$hash_vars$all_counts_for_plotting[['limma']]$logFC, 
	y=-log10(plotter$hash_vars$all_counts_for_plotting[['limma']]$adj.P.Val),
	main= c('groupsB-groupsA'), xlab='logFC', ylab='-log10 adjusted p-value',
	col=c('blue', 'red')[limma_sig],
	pch=20,cex.lab=1.6, cex.axis=1.5)
abline(v= plotter$hash_vars$opt$lfc, col='cyan')	
abline(v= -(plotter$hash_vars$opt$lfc), col='cyan')	
abline(h=-log10(plotter$hash_vars$opt$p_val_cutoff), col='red', lty='dashed')

```

```{r limma_conditional_block_text, eval='limma' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}
...
```{r NOISeq_conditional_block_head, eval='NOISeq' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}
...
```{r NOISeq_conditional_block_plot, eval='NOISeq' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, warning=FALSE, message=FALSE, results='hide',}
    q_value <- 1-plotter$hash_vars$opt$p_val_cutoff # Differential expression threshold for creating plot
	NOISeq::DE.plot(plotter$hash_vars$package_objects[['NOISeq']], q = q_value, graphic = 'MD', cex.lab=1.4, cex.axis=1.4)

```

```{r NOISeq_conditional_block_text, eval='NOISeq' %in% names(plotter$hash_vars$all_counts_for_plotting), echo=FALSE, results='asis'}
...
df$gene_names <- row.names(df)
df_melted_pval <- reshape2::melt(df, id.vars = "gene_names", measure.vars = plotter$hash_vars$final_FDR_names)
df_melted_FDR <- reshape2::melt(df, id.vars = "gene_names", measure.vars = plotter$hash_vars$final_pvalue_names)
# Now plot
...
```{r pval_corr, echo = FALSE, warning = FALSE, eval = length(plotter$hash_vars$all_counts_for_plotting) > 1}

PerformanceAnalytics::chart.Correlation(df[, c("combined_FDR", plotter$hash_vars$final_FDR_names)], histogram=TRUE, pch=19, log="xy", na.action = na.omit)
PerformanceAnalytics::chart.Correlation(df[, plotter$hash_vars$final_pvalue_names], histogram=TRUE, pch=19, log="xy", na.action = na.omit)
PerformanceAnalytics::chart.Correlation(df[, c("mean_logFCs", plotter$hash_vars$final_logFC_names)], histogram=TRUE, pch=19, log="xy", na.action = na.omit)
