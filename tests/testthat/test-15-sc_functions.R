...
test_that("get_sc_markers works as intended, DEG TRUE", {
  ...
  markers_1 <- data.frame(p_val = c(0.30, 0.33, 0.70, 1),
           avg_log2FC = c(-7.1, 9.9, -1.9, 3.5), pct.1 = c(0, 0.40, 0.20, 0.20),
           pct.2 = c(0.33, 0, 0.33, 0.33), p_val_adj = 1,
  ...
  markers_global <- data.frame(p_val = c(0.27, 0.28, 0.41, 0.77),
    avg_log2FC = c(10, -6.9, 3.9, -1.7), pct.1 = c(0.22, 0, 0.33, 0.11),
    pct.2 = c(0, rep(0.17, 3)), p_val_adj = 1,
...
test_that("test process_sc_params, per sample annotation mode", {
  ref_filter <- system.file("external_data/test_filter.txt",
