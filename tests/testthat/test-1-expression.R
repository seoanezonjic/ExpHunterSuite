# Broken test due to bad design. Might work in installed form, ask #pedro (it's looking for stable installed version template while using htmlreportR
# report function)
# test_that("main expression function works", {
#   # This test has bad design. It uses find.package to define template folder, which will yield different results
#   # depending on the environment running this test. Interactive testing will yield currently loaded version, running test
#   # suite will point to installed package instead of package that you are trying to test.
