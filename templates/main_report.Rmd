...
```{r WGCNA_conditional_block_head_MMs, eval=grepl("W", opt$modules), echo=FALSE, results='asis'}
	...
	cat("Finally, there is a plot that shows, for each gene, the cluster ID ascertained by WGCNA, vs. the cluster whose eigen gene has the highest correlation.")

```

```{r WGCNA_conditional_block_plot_MMs, eval=grepl("W", opt$modules), echo=FALSE, warning=FALSE, message=FALSE, fig.height=10, fig.width=15, results="asis"}
	pp<- results_WGCNA[['plot_objects']][['cluster_vs_MM']]
	plot_in_div(pp)
```
...
```{r WGCNA_conditional_block_plot_factors, eval=grepl("W", opt$modules), echo=FALSE, warning=FALSE, message=FALSE, fig.height=10, fig.width=15, results = 'asis'}
  ...
  #  plot_in_div(replayPlot(results_WGCNA[['plot_objects']][['trait_vs_module_heatmap']]), calc_height_clusters(cluster_count), calc_width_clusters(features_count))
	...
	plot_in_div(replayPlot(results_WGCNA[['plot_objects']][['trait_and_module_dendogram']]),
		fig_width=calc_width_clusters((features_count + cluster_count), multiplier = 0.15))
    plot_in_div(replayPlot(results_WGCNA[['plot_objects']][['trait_and_module_heatmap']]),
    	calc_height_clusters(features_count + cluster_count),
    	calc_width_clusters(features_count + cluster_count))
