...
Various tables of results for WGCNA. Will shortly be changed for some heatplots and more informative graphics:

In total there was a number of ", max(unique(results_WGCNA$gene_cluster_info$MM_Cluster_ID)) ,"clusters."
...
```{r WGCNA_conditional_block_plot, eval=grepl("W", opt$modules), echo=FALSE, warning=FALSE, message=FALSE, fig.height=12, fig.width=15}
	df <- table(results_WGCNA$gene_cluster_info$MM_Cluster_ID)
	ggplot(as.data.frame(df), aes(Var1, Freq)) + 
    geom_bar(stat="identity") + labs(x = "Cluster ID (0 = unclustered)", y = "Frequency", main="Number of Genes per Cluster")
...
```{r PCIT_conditional_block_plot, eval=grepl("P", opt$modules), echo=FALSE, warning=FALSE, message=FALSE}
		ggplot(metrics_WGCNA, aes(x=ctrl_cn)) +
                  geom_density()
        ggplot(metrics_WGCNA, aes(x=treat_cn)) +
                  geom_density()
        ggplot(metrics_WGCNA, aes(x=diff_cn)) +
                geom_density()
        ggplot(metrics_WGCNA, aes(x=diffK)) +
                geom_density()
        ggplot(metrics_WGCNA, aes(x=de)) +
                geom_density()
        ggplot(metrics_WGCNA, aes(x=de, y=diffK)) +
                geom_point()
        ggplot(metrics_WGCNA, aes(x=pif)) +
                geom_density()
        ggplot(metrics_WGCNA, aes(x=rif)) +
                geom_density()
        ggplot(metrics_WGCNA, aes(x=de, y=rif)) +
                geom_point()
