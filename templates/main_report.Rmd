...
```{r DESeq2_conditional_block_head, eval='DESeq2' %in% names(all_counts_for_plotting), echo=FALSE, results='asis'}
...
```{r DESeq2_conditional_block_plot, eval='DESeq2' %in% names(all_counts_for_plotting), echo=FALSE}
	plotMA(package_objects[['DESeq2']], cex.lab=1.6, cex.axis=1.5)
```

```{r DESeq2_conditional_block_text, eval='DESeq2' %in% names(all_counts_for_plotting), echo=FALSE, results='asis'}
...
This is the volcano plot of log-fold changes versus log-odds (log10 of adjusted P values) of differential expression from the limma package:")

```

```{r limma_conditional_block_plot, eval='limma' %in% names(all_counts_for_plotting), echo=FALSE, warning=FALSE}

limma_sig <- DE_all_genes$limma_DEG+1
plot(x=all_counts_for_plotting[['limma']]$logFC, y=-log10(all_counts_for_plotting[['limma']]$P.Value), xlab='logFC', ylab='logOdds', col=c('blue', 'red')[limma_sig], pch=20, main= c('groupsB-groupsA'), cex.lab=1.6, cex.axis=1.5)
abline(v= opt$lfc, col='cyan')	
limit.pval_limma <- -log10(max(all_counts_for_plotting[['limma']]$P.Value)) 
...
DE.plot(package_objects[['NOISeq']], q = opt$q_value, graphic = 'MD', cex.lab=1.4, cex.axis=1.4)
