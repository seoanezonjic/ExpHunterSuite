...
targetcols <- intersect(c("rif1","rif2","diffK","diff_cn"),colnames(DEGH_results_Symbol))

targetindexes <- lapply(targetcols,function(tcol){
	# Sort column
	csorted <- sort(DEGH_results_Symbol[,tcol], decreasing = TRUE)
	# Take tops
	top15 <- head(csorted,15)
	bot15 <- tail(csorted,15)
	# Find
	topindx <- which(DEGH_results_Symbol[,tcol] %in% top15)
	botindx <- which(DEGH_results_Symbol[,tcol] %in% bot15)
	# Retrun
	return(list(top = topindx, bot = botindx))
})
# Prepare pattern based data
patterncols <- colnames(DEGH_results_Symbol)[grepl("^logFC",colnames(DEGH_results_Symbol))]
patternindexes <- lapply(patterncols,function(pcol){
	# Filter
	to_remove <- which(DEGH_results_Symbol[,gsub("logFC","FDR",pcol)] > degh_exp_threshold)
	# Sort column
	if(length(to_remove) > 0){
		csorted <- sort(DEGH_results_Symbol[-to_remove,pcol], decreasing = TRUE)
	}else{
		csorted <- sort(DEGH_results_Symbol[,pcol], decreasing = TRUE)		
	}
	# Take tops
	top15 <- head(csorted,15)
	bot15 <- tail(csorted,15)
	# Find
	topindx <- setdiff(which(DEGH_results_Symbol[,pcol] %in% top15),to_remove)
	botindx <- setdiff(which(DEGH_results_Symbol[,pcol] %in% bot15),to_remove)
...
# Add special case
	# Filter
	to_remove <- which(DEGH_results_Symbol[,"combined_FDR"] > degh_exp_threshold)
	# Sort column
	if(length(to_remove) > 0){
		csorted <- sort(DEGH_results_Symbol[-to_remove,"mean_logFCs"], decreasing = TRUE)
	}else{
		csorted <- sort(DEGH_results_Symbol[,"mean_logFCs"], decreasing = TRUE)		
	}
	# Take tops
	top15 <- head(csorted,15)
	bot15 <- tail(csorted,15)
	# Find
	topindx <- setdiff(which(DEGH_results_Symbol[,"mean_logFCs"] %in% top15),to_remove)
	botindx <- setdiff(which(DEGH_results_Symbol[,"mean_logFCs"] %in% bot15),to_remove)
...
# Take targets
topgenes <- DEGH_results_Symbol[topindx,c("Symbol",targetcols,patterncols,"mean_logFCs",colnames(DEGH_results_Symbol)[1])]
botgenes <- DEGH_results_Symbol[botindx,c("Symbol",targetcols,patterncols,"mean_logFCs",colnames(DEGH_results_Symbol)[1])]
# Sort
