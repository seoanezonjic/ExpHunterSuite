...
```{r check_kegg, echo=FALSE, results='asis', eval=kegg_flags$KEGG_SECTION}
cat("## **KEGG analysis**")
```


```{r check_kegg_old, echo = FALSE, eval=FALSE}
#XQ NO USAR SOLUCION ANTERIOR?
...
```{r plot_kegg,  results='asis', echo = FALSE}
...
if(go_flags[["enrich_go"]]){
	...
	for(i in seq_along(enrich_go)){
		...
		if(go_flags[["enrich_go_gsea"]]){
			if(nrow(enrich_go_gsea[[i]]@result) > 0){
				go_pp3 <- dotplot(enrich_go_gsea[[i]], showCategory=30) + ggtitle("dotplot for GSEA")
				go_pp5 <- cnetplot(enrich_go_gsea[[i]])
				go_pp7 <- emapplot(enrich_go_gsea[[i]])
				go_pp9 <- heatplot(enrich_go_gsea[[i]])
				go_pp11 <- ridgeplot(enrich_go_gsea[[i]])
				go_pp12 <- gseaplot2(enrich_go_gsea[[i]], geneSetID=1:4, title = enrich_go_gsea[[i]]$Description[1], pvalue_table = TRUE) 
				go_pp13 <- gsearank(enrich_go_gsea[[i]], geneSetID=1, title = enrich_go_gsea[[i]]$Description[1])
			}
		}
