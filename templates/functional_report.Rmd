...
calc_width <- function(enrich_obj, showCategory = 30, category_character_size = 0.035, character_width = 3, legend_size = 1){
	...
	cat(paste0("Real width is ", width_size, " inches"))
  	...
  	message(paste0("Used width of ", width_size, " inches"))
...
cl_table <- as.data.frame(table(DEG_annot_table$genes_tag))
names(cl_table) <- c("Gene_tag","Genes")
knitr::kable(cl_table)
# knitr::kable(table(DEG_annot_table$genes_tag))
```

```{r table_top_genes, echo = FALSE, eval = exists("DEG_annot_table_Symbol"), results = 'asis'}

# Prepare regular data
targetcols <- c("rif1","rif2","diffK","diff_cn")
targetindexes <- lapply(targetcols,function(tcol){
	# Sort column
	csorted <- sort(DEG_annot_table_Symbol[,tcol], decreasing = TRUE)
	# Take tops
	top15 <- head(csorted,15)
	bot15 <- tail(csorted,15)
	# Find
	topindx <- which(DEG_annot_table_Symbol[,tcol] %in% top15)
	botindx <- which(DEG_annot_table_Symbol[,tcol] %in% bot15)
	# Retrun
	return(list(top = topindx, bot = botindx))
})
# Prepare pattern based data
patterncols <- colnames(DEG_annot_table_Symbol)[grepl("^logFC",colnames(DEG_annot_table_Symbol))]
patternindexes <- lapply(patterncols,function(pcol){
	# Filter
	to_remove <- which(DEG_annot_table_Symbol[,gsub("logFC","FDR",pcol)] > degh_exp_threshold)
	# Sort column
	if(length(to_remove) > 0){
		csorted <- sort(DEG_annot_table_Symbol[-to_remove,pcol], decreasing = TRUE)
	}else{
		csorted <- sort(DEG_annot_table_Symbol[,pcol], decreasing = TRUE)		
	}
	# Take tops
	top15 <- head(csorted,15)
	bot15 <- tail(csorted,15)
	# Find
	topindx <- setdiff(which(DEG_annot_table_Symbol[,pcol] %in% top15),to_remove)
	botindx <- setdiff(which(DEG_annot_table_Symbol[,pcol] %in% bot15),to_remove)
...
# Add special case
	# Filter
	to_remove <- which(DEG_annot_table_Symbol[,"combined_FDR"] > degh_exp_threshold)
	# Sort column
	if(length(to_remove) > 0){
		csorted <- sort(DEG_annot_table_Symbol[-to_remove,"mean_logFCs"], decreasing = TRUE)
	}else{
		csorted <- sort(DEG_annot_table_Symbol[,"mean_logFCs"], decreasing = TRUE)		
	}
	# Take tops
	top15 <- head(csorted,15)
	bot15 <- tail(csorted,15)
	# Find
	topindx <- setdiff(which(DEG_annot_table_Symbol[,"mean_logFCs"] %in% top15),to_remove)
	botindx <- setdiff(which(DEG_annot_table_Symbol[,"mean_logFCs"] %in% bot15),to_remove)
...
# Take targets
topgenes <- DEG_annot_table_Symbol[topindx,c("Symbol",targetcols,patterncols,"mean_logFCs",colnames(DEG_annot_table_Symbol)[1])]
botgenes <- DEG_annot_table_Symbol[botindx,c("Symbol",targetcols,patterncols,"mean_logFCs",colnames(DEG_annot_table_Symbol)[1])]
# Sort

# Message
cat("## **Top genes**\n")
cat(paste(c("Table of signifcant genes. Variables taken into account are being shown into table (except gene symbols/ID). 15Top of best (",nrow(topgenes)," items) and worst (",nrow(botgenes)," items) are being shown in two different tables\n"), collapse = ""))

```
```{r render_topgenes, echo = FALSE, eval = exists("DEG_annot_table_Symbol"), results = 'asis'}
cat("\n### **Top positive**\n")
# Render
datatable(topgenes, filter = 'top', rownames = FALSE, extensions = c('Buttons','ColReorder'),
                    options = list(
                    	colReorder = TRUE,
                    	dom = 'lftBip',
...
```
```{r render_botgenes, echo = FALSE, eval = exists("DEG_annot_table_Symbol"), results = 'asis'}
cat("\n### **Top negative**\n")
# Render
datatable(botgenes, filter = 'top', rownames = FALSE, extensions = c('Buttons','ColReorder'),
                    options = list(
                    	colReorder = TRUE,
                    	dom = 'lftBip',
                        buttons = c('copy', 'csv', 'excel')
...
if(!is.null(custom_enrichments)){
	...
	# Check all variables
	invisible(lapply(custom_enrichments,function(var){
		custom_flags[var$File] <<- check_results(var$Result)
	}))
...
invisible(lapply(seq_along(custom_enrichments),function(i){
	...
	if(!custom_flags[[res$File]]){
		...
		# Prepare ORA plots
		body <- "enr_custom <- custom_enrichments[[{{i}}]]$Result
				default_width <- set_default_width(enrich_obj = enr_custom, showCategory=n_category)
				
				# Header
				cat(paste('\n## **',basename(custom_enrichments[[{{i}}]]$File),'**\n',sep=''))
				...
				enr_custom <- setReadable(enr_custom, OrgDb = biomaRt_organism_info$Bioconductor_DB[1], keyType='ENTREZID')
