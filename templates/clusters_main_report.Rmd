...
```{r kegg_main,  results='asis', echo = FALSE, eval = executed$Executed["KEGG"]}
cat("## **KEGG analysis**\n")
```
```{r kegg_ora, results='asis', echo=FALSE, eval = executed$Executed["KEGG"] & exists("enrichments_ORA")}

# default_width <- set_default_width(enrich_obj = enrichments_ORA$KEGG, showCategory=n_category)
longer_category <- max(nchar(enrichments_ORA$KEGG@compareClusterResult$Description))
if(longer_category > 30) default_width <- longer_ca12 + 0.04 * longer_category
...
if(longer_category > 30){plot_in_div(pp, width = default_width)
}else{plot(pp)}
