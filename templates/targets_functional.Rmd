...
if ("g" %in% nomenclatures) {
  any_go <- any(unlist(lapply(names(enrich_GO), function(onto){ check_results(enrich_GO[[onto]]) })))
}
...
```{r kegg_ora, results='asis', echo = FALSE, eval = "K" %in% nomenclatures, message=FALSE, warning=FALSE}
...
```{r display_kegg_res, results='asis', echo = FALSE, eval = "K" %in% nomenclatures}
...
```{r plot_go_cl, results='asis', echo = FALSE, eval = ("g" %in% nomenclatures && any_go), message=FALSE, warning=FALSE}
...
```{r react_ora, results='asis', echo = FALSE, eval = ("R" %in% nomenclatures && !length(get_categories(enrich_react)) == 0), message=FALSE, warning=FALSE}
